...
TODO: round the corners of the spine a bit, perhaps add a gutter to the spine
TODO: make the louver wings a bit narrower (closer to just tire width)
TODO: add an option for a reduced angle (parallel to ground on all louvers) on the louver wings, perhaps with longer chord on the louvers then
TODO: add a slight twist to the louver wish to create a small amount of outwash
"""
import math
...
except Exception:  # pragma: no cover - optional dependency for cap triangulation
    Delaunay = None
...
def write_stl(
    ...
    if validate_and_repair and HAS_PYVISTA:
        ...
        with open(filename, "wb") as f:
            # 80 byte header
            f.write(b"Generated by louver_fender.py" + b" " * (80 - len("Generated by louver_fender.py")))
            # Number of triangles
...
def calculate_front_airfoil_thickness(
    ...
    1. Provides sufficient inner clearance for the tire to pass through
    ...
    min_structural_half = required_inner_half + front_wall_thickness_mm
...
def build_fender(
    ...
    if front_airfoil_thickness_mm is not None:
        ...
        print(f"    Min structural half: {thickness_details['min_structural_half_mm']:.1f} mm")
        print(f"  Result:")
        print(f"    Constraint: {thickness_details['constraint'].upper()}")
        print(f"    Full thickness: {front_thickness_mm:.1f} mm")
        print(f"    Actual t/c ratio: {thickness_details['actual_thickness_ratio']:.1%}")
...
if __name__ == "__main__":
    ...
    build_fender(
        ...
        front_airfoil_chord_mm=70.0,  # 70mm chord length pointing radially inward toward wheel
        front_airfoil_thickness_ratio=0.18,  # Target 18% t/c ratio (actual may be higher for tire clearance)
        ...
        front_airfoil_camber=0.0,  # 0% camber for symmetric airfoil (no camber)
        front_airfoil_camber_position=0.35,  # 35% camber position (not used when camber=0)
        front_airfoil_kammback_start=0.75,  # 75% Kammback cut for blunt trailing edge
        front_wall_thickness_mm=3.0,  # 3mm shell wall on the hollow front airfoil
        front_airfoil_tire_clearance_mm=4.0,  # 4mm clearance between tyre and inner wall
